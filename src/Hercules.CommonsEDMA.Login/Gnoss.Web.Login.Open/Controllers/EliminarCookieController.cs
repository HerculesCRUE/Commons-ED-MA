        ...
        public void Index()
        {
            ...
            if (!Request.Headers.ContainsKey("eliminar") && !Request.Query.ContainsKey("eliminar"))
            {
                ...
                if (Request.Cookies.ContainsKey(cookieEnvioKey) || Request.Headers.ContainsKey("nuevoEnvio"))
                {
                    ...
                    mHttpContextAccessor.HttpContext.Response.Cookies.Append(cookieEnvioKey, "true", new CookieOptions { Expires = DateTime.Now.AddDays(1),Secure=true });
            ...
            else
            {
                ...
                {
                    Response.Cookies.Append(cookieUsuarioKey, "0", new CookieOptions { Expires = DateTime.Now.AddDays(-1),Secure=true });
                }
                ...
                {
                    Response.Cookies.Append(cookieRewriteKey, "0", new CookieOptions { Expires = DateTime.Now.AddDays(-1) , Secure = true });
                }

                if (Request.Cookies.ContainsKey(cookieEnvioKey))
                {
                    Response.Cookies.Append(cookieEnvioKey, "0", new CookieOptions { Expires = DateTime.Now.AddDays(-1) , Secure = true });
                }

                if (Request.Cookies.ContainsKey("redireccion"))
                {
                    Response.Cookies.Append("redireccion", "0", new CookieOptions { Expires = DateTime.Now.AddDays(-1) , Secure = true });
                }

                string cookieTokenKey = "tokenDeVuelta";
                if (Request.Cookies.ContainsKey(cookieTokenKey))
                {
                    Response.Cookies.Append(cookieTokenKey, "0", new CookieOptions() { Expires = DateTime.Now.AddDays(-1) ,Secure=true});
                }
